...
namespace Walmart.Sdk.Base.Http.Fetcher
{
	...
	public class LocalFetcher : BaseFetcher
	{
		...
		{
			Assembly assembly = config.GetType().GetTypeInfo().Assembly;
			var assemblyName = assembly.GetName().Name;
			using (Stream stream = assembly.GetManifestResourceStream(assemblyName + "." + assemblyPath + "." + filename))
			{
				try
				{
		...
		override public Task<IResponse> ExecuteAsync(IRequest request)
		{
			...
			{
				int.TryParse(filename.Substring(filename.Length - 9, 3), out code);
			}

			Response response = BuildResponseObject(content, (System.Net.HttpStatusCode)code);
