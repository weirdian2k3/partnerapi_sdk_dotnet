...
namespace Walmart.Sdk.Marketplace.IntegrationTests.V3
{
	...
	public class OrderEndpointTests : BaseIntegrationTest
	{
		...
		{
			Order result = await orderApi.CancelOrderLines("test", GetRequestStub("V3.requestStub.cancelOrderLines"));
			Assert.IsType<Order>(result);
		}
		...
		{
			Order result = await orderApi.RefundOrderLines("test", GetRequestStub("V3.requestStub.refundOrderLines"));
			Assert.IsType<Order>(result);
		}
		...
		{
			Order result = await orderApi.ShippingUpdates("test", GetRequestStub("V3.requestStub.shippingUpdates"));
			Assert.IsType<Order>(result);
		}
