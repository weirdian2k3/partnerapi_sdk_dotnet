...
namespace Walmart.Sdk.Marketplace.E2ETests.V3
{
	...
	public class PromotionEndpointTests : BaseE2ETest
	{
		...
		{
			using (Stream stream = LoadRequestStub("V3.requestStub.promotionFeed"))
			{
		...
		private Stream GetRequestContentForPromotionUpdate(string sku, string type, string processMode)
		{
			...
			DateTime date1 = DateTime.UtcNow.AddMonths(2);
			//.ToString("yyyy'-'MM'-'dd'T'HH':'mm':'ss'.'fff'Z'");
			DateTime date2 = date1.AddDays(7);
			DateTime date3 = date1.AddDays(10);
			DateTime date4 = date1.AddDays(20);
			content = content
			    .Replace("{{sku}}", sku)
			    .Replace("{{type}}", type)
		...
		private string GetPayloadForDelete(string text, string sku)
		{
			...
			else
			{
				...
				var sPattern = "[a-z0-9]{8}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{4}-[a-z0-9]{12}";
				...
				var prefix = "<?xml version=\"1.0\" encoding=\"UTF - 8\" standalone=\"yes\"?>\r\n<Price xmlns=\"http://walmart.com/\">\r\n<itemIdentifier>\r\n<sku>"
				    + sku + "</sku>\r\n</itemIdentifier>\r\n<pricingList>\r\n";
				var suffix = "</pricingList>\r\n</Price>";
