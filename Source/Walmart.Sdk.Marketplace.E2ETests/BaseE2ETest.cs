...
namespace Walmart.Sdk.Marketplace.E2ETests
{
	public abstract class BaseE2ETest
	{
		...
		{
			Assembly assembly = typeof(BaseE2ETest).GetTypeInfo().Assembly;
		...
		protected void LoadRawSettings()
		{
			...
			using (Stream stream = LoadStreamFromAssembly("settings.json"))
			{
			...
			using (StreamReader credsStream = File.OpenText("credentials.json"))
			{
